...
import com.linhavital.app.data.model.ConfiguracaoMonitoramentoRequest
...
class MonitoramentoRepository {
    ...
    suspend fun obterStatus(
        usuarioId: Long
    ): Result<MonitoramentoStatus> =
        runCatching {
            api.getMonitoramentoStatus(usuarioId)
        }
    ...
    suspend fun configurar(
        ...
        runCatching {
            api.configurarMonitoramento(
                usuarioId,
                ConfiguracaoMonitoramentoRequest(
                    ativo = ativo,
    ...
    suspend fun checkIn(
        ...
        runCatching {
            api.registrarCheckIn(usuarioId)
        }
}